...
from app.routes.user_me_route import (
...
def test_read_users_me_admin(
    ...
    response = client.get(route_paths.ROUTE_USER_ME,
...
def test_read_users_me_normal(client: TestClient, db: Session) -> None:
    ...
    response = client.get(route_paths.ROUTE_USER_ME, headers=headers)
    assert is_success_code_response(response)
...
def test_update_me_full_name(
    ...
    response = client.put(
        route_paths.ROUTE_USER_ME, headers=user_token_headers, json=data)
